...
public class MultiplayerMenu : MonoBehaviour
{
	...
	public void Connected(NetWorker networker)
	{
        ...
        if (NetworkManager.Instance.Networker is IServer)
        {
            ...
            NetworkManager.Instance.InstantiateGameManager().networkStarted += OnGameManagerInited;
        } else {
            InitGameScene();
        }
    ...
    {
        SceneManager.sceneLoaded -= OnSceneLoaded;
        GameManagerNetworked.Instance.Init();
    }
    ...
    private void InitGameScene()
    {
        if (NetworkManager.Instance.Networker is IServer)
		{
			if (!DontChangeSceneOnConnect)
				...
				NetworkObject.Flush(NetworkManager.Instance.Networker); //Called because we are already in the correct scene!
		}
    }
